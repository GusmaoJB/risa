risaplot <- function(x, area_of_interest = TRUE, raw_ggplot = FALSE) {
  ...
  .get_raster <- function(obj, object_name = "raster") {
  ...
  .rast_list_to_df <- function(r_list,
    ...
    r_stack <- terra::rast(
      purrr::map(group_names, \(nm) .get_raster(r_list[[nm]], object_name))
    )
  ...
  .join_shps <- function(r_list, group_col = "group") {
    purrr::imap(
      ...
      \(obj, nm) {
        ...
        dplyr::mutate(obj$shp, !!group_col := nm)
      }
    ) |>
      dplyr::bind_rows()
  }
  ...
  .make_aoi <- function(aoi, xy_df, crs_like) {
    if (!area_of_interest) {
      return(NULL)
    }
    
    .ensure_aoi_layer(aoi, xy_df = xy_df, crs_like = crs_like)
  }
  ...
  if (inherits(x, "risaMaps")) {
    ...
    if (raster_mode) {
      spp_df <- .rast_list_to_df(sp_list, object_name = "raster", value_name = "value")
      stressor_df <- .rast_list_to_df(st_list, object_name = "raster", value_name = "value")
      ...
      aoi_layer <- .make_aoi(
        x$area_of_interest,
        xy_df = spp_df,
        crs_like = .get_raster(sp_list[[1]], "raster")
      )
  ...
  # ---------------------------------------------------------------------------
  if (depth == 2) {
    risk_raw <- .rast_to_df(x$total_raw, value_name = "Risk")
  ...
  # risaHRA depth > 2
  # ---------------------------------------------------------------------------
  eco_risk_raw <- .rast_to_df(x$ecosys_risk_raw, value_name = "Risk")
  ...
  raw_risk_df <- purrr::map_dfr(
    species_names,
    \(sp) {
      .rast_to_df(x[[sp]]$total_raw, value_name = "Risk") |>
        dplyr::mutate(species = sp)
    }
  )
  ...
  recl_risk_df <- purrr::map_dfr(
    species_names,
    \(sp) {
      .rast_to_df(x[[sp]]$total_reclassified, value_name = "Risk (reclass.)") |>
        dplyr::mutate(
          species = sp,
